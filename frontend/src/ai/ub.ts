...
// Time-of-day greeting word from the LOCAL clock. Night counts as "Good evening".
// (Used by the client-side fallback when the local UB brain is unavailable; the
// backend brain computes its own greeting from injected runtime context.)
...
// Professional Union Bank reception greeting — composed, time-appropriate, never
// a bare "hi". Mirrors the officer persona used by the backend assistant.
export function buildGreeting(ctx: { hasFraud: boolean; clusters: number }): string {
  ...
  const status = ctx.hasFraud
    ? `Fraud monitoring is active — ${ctx.clusters} cluster${ctx.clusters !== 1 ? 's' : ''} currently flagged for review.`
    : 'The platform is online and ready for analysis.'
  return `${hello}. Welcome to the Union Bank TGIE demonstration. I am the bank's AI investigation assistant. ${status}`
}
